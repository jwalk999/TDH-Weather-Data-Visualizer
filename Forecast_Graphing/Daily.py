...
# Write the formatted forecast data to the daily forecast text file.
with open("Data/daily_forecast.txt", "w+", encoding="utf-8") as f:
    
    # Write location information before the forecast table.
    f.write(
        f"Coordinates: "
        f"{response_daily.Latitude()}°N , "
        f"{response_daily.Longitude()}°E"
    )

    f.write(
        f"\nElevation: "
        f"{response_daily.Elevation()}m above sea level"
    )

    f.write(
        f"\nTimezone: "
        f"{response_daily.Timezone()}"
    )

    # Add a heading before the daily forecast.
    f.write("\n~~~Daily Data~~~\n")
